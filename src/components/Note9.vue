<template>
  <div class="box">
    ...
    <h2>Le Do</h2>
    <audio src="@/assets/Note/Do.mp3" controls></audio>
    <div id="note_portee_DoA"></div>
    <div class="boutons">
      <div class="left">
        <p class="link" @click="nav('note')">Retour au menu</p>
      </div>
      <div class="right">
        <p class="link" @click="nav('Note7')">Page précédente</p>
        <p class="link" @click="nav('note')">Finir le cours</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    // on place la portée
    const monDiv = document.getElementById("note_portee_DoA");
    ...
    notesMesure_2[3].setStyle({ fillStyle: "red", strokeStyle: "red" });
    ...
    var textLa = new Vex.Flow.TextNote({
      text: "La",
      font: {
        family: "Arial",
        size: 14,
    ...
    var textSi = new Vex.Flow.TextNote({
      text: "Si",
      font: {
        family: "Arial",
        size: 14,
    ...
    var textDoA = new Vex.Flow.TextNote({
      text: "Ceci est un Do aigu",
      font: {
        family: "Arial",
        size: 14,
        weight: "",
    ...
    maVoix4.addTickables([textLa]);
    maVoix4.addTickables([textSi]);
    maVoix4.addTickables([textDoA]);
    ...
    textLa.setContext(monContexte).draw();
    textSi.setContext(monContexte).draw();
    textDoA.setContext(monContexte).draw();
  },
};
</script>
