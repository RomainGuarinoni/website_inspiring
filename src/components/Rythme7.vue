<template>
  <div class="box">
    ...
    <h3>La demi-pause</h3>
    <p>
      A chaque rythme, vu précédemment, on peut associer un silence de durée
      similaire. <br />Pour 2 pulsations, on utilise la demi-pause. Sur une
      portée, elle est "posée" sur la 3e ligne.
    </p>
    <div id="rythme_portee6"></div>
    <div class="boutons">
      <div class="left">
        <p class="link" @click="nav('rythme')">Retour au menu</p>
      </div>
      <div class="right">
        <p class="link" @click="nav('rythme6')">Page précédente</p>
        <p class="link" @click="nav('rythme8')">Page suivante</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    // on place la portée
    const monDiv = document.getElementById("rythme_portee6");
    ...
    /////Mesure n°1////
    //////////////////
    var Mesure_1 = new Vex.Flow.Stave(220, 0, 310);
    ...
    maVoix2.addTickables([text11]);
    maVoix2.addTickables([text12]);
    maVoix2.addTickables([text13]);
    maVoix2.addTickables([text14]);
    ...
    text11.setContext(monContexte).draw();
    text12.setContext(monContexte).draw();
    text13.setContext(monContexte).draw();
    text14.setContext(monContexte).draw();

    ///////////////////
    /////Mesure n°2////
    ///////////////////

    var Mesure_2 = new Vex.Flow.Stave(590, 0, 330); //attention à bien modifier la première valeur
    ...
    text21.setContext(monContexte).draw();
    text22.setContext(monContexte).draw();
    text23.setContext(monContexte).draw();
    text24.setContext(monContexte).draw();
  },
};
</script>
