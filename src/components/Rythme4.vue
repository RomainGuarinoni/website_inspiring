<template>
  <div class="box">
    ...
    <h3>La Ronde</h3>
    <p>
      La ronde dure 4 temps, soit l'entièreté de notre mesure. Pour la jouer, il
      faut la tenir pendant les 4 temps.<br />
    </p>
    <div id="rythme_portee3"></div>
    <div class="boutons">
      <div class="left">
        <p class="link" @click="nav('rythme')">Retour au menu</p>
      </div>
      <div class="right">
        <p class="link" @click="nav('rythme3')">Page précédente</p>
        <p class="link" @click="nav('rythme5')">Page suivante</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    // on place la portée
    const monDiv = document.getElementById("rythme_portee3");
