<template>
  <div class="box">
    ...
    <h3>La noire</h3>
    <p>
      Le rythme le plus simple est la noire : il est égal à 1 temps. Ainsi,
      lorsqu'on joue une noire, il s'écoule une pulsation : <br />
    </p>
    <div id="rythme_portee1"></div>
    <div class="boutons">
      <div class="left">
        <p class="link" @click="nav('rythme')">Retour au menu</p>
      </div>
      <div class="right">
        <p class="link" @click="nav('Rythme1')">Page précédente</p>
        <p class="link" @click="nav('rythme3')">Page suivante</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    // on place la portée
    const monDiv = document.getElementById("rythme_portee1");
